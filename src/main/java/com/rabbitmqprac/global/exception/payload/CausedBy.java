...
public record CausedBy(
    ...
    public CausedBy {
        ...
        if (!isValidCodes(statusCode.getCode(), reasonCode.getCode(), domainCode.getCode())) {
     ...
     * @return CausedBy
     * @throws IllegalArgumentException 전체 코드가 5자리가 아닌 경우, 혹은 각 상태 코드가 자릿수를 준수하지 않은 경우
    ...
    private boolean isValidCodes(int statusCode, int reasonCode, int domainCode) {
        return isValidDigit(statusCode, 3)
                && isValidDigit(reasonCode, 1)
                && isValidDigit(domainCode, 1);
    }
